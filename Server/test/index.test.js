...
const correctEmail = "example@mail.com";
...
describe("Tests de Rutas", () => {
  ...
  describe("GET /rickandmorty/login", () => { it("Responde con status:200", async () => {
    await agent .get(`/rickandmorty/login?email=${correctEmail}&password=${correctPassword}`) .expect(200);
    });
    it("Responde con status:500", async () => {
    await agent .get(`/rickandmorty/login?email=${falseEmail}&password=123`) .expect(200);
    }); });

    describe("POST /rickandmorty/fav", () => {
        it("Responde con un arreglo correctamente", async() => {
            await agent.post("/rickandmorty/fav").send({character: rickMock})
            const response = await agent.post("/rickandmorty/fav").send({character: mortyMock})
    ...
    describe("DELETE /rickandmorty/fav/:id", () => { 
        ...
        expect(response.body.length).toBe(1); expect(response.body).toEqual([ mortyMock ]);
        }) })
});
